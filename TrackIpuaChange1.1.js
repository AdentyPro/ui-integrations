...
setTimeout(async () => {
  ...
  if (window.aidpSCookieListPromise) {
    trc('Promise exist. wait. ip ua change js ');
    window.aidpSCookieListPromise.then(useData).catch(console.error);
    return;
  }

  let resolveFn, rejectFn;
  trc('Promise init. ip ua change js');
  ...
  if (window.aidpSCookieList) {
    trc('window global cookie exist. ip ua change js');
    resolveFn(window.aidpSCookieList);
  } else {
    window.adenty?.scookie?.get()
    .then(data => {
      trc('Promise resolve success. ip ua change js');
      resolveFn(data);
    })
    .catch(error => {
      trc('Promise resolve error. ip ua change js');
      resolveFn([]);
    });
  }
 
  trc('Promise wait. ip ua change js');
  window.aidpSCookieListPromise.then(useData).catch(console.error);
